...
;;    (global-visual-line-mode 1)  ;; Stop wrapping of text in org mode
    (setq org-file-apps
          (append '(
                    ("\\.pdf\\'" . "evince %s")
...
* magit
To open magit for a file : C-x g
to show shortcut in magit type : h

To Unstage : u or U
To stage : s or S
To commit : press C > Enter to Commit file > Write commit msg > C-c C-c to save and exit the commit
To push   : press P > press u > github usr/pwd
